...
function LoadingView({ tokenIds }: IStatusView) {
    return (
        <div className="flex flex-col gap-4 items-center text-center mb-8">
            ...
            <h3 className="font-semibold uppercase text-slate-900">{tokenIds.map((bn, key) => <span key={key}>{bn.toNumber()}</span>)}</h3>
                ...
                {status === "idle" &&
                    <>
                        <div className='text-center'>
                            <h3 className="font-semibold uppercase text-slate-400">{tokenIds?.map((value, index) => <div key={index}>{value.toString()}</div>)}</h3>
                        </div>
                        <button
                            type="button"
